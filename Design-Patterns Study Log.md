...
- **简单工厂模式** ：
  - 用来生产同一等级结构的**任意**产品。
  - 对于增加新的产品，需要去覆盖/扩展已有代码
  - 不符合设计原则，但实际业务使用最多！
- **工厂方法** ：
  - 生产同一等级结构的**固定**产品。
  - 支持增加任意产品，（但每写一个新产品就要再加一个对应的工厂类来实现开闭原则）。
  - 虽然符合设计原则，但代码量会上升。
- **抽象工厂模式**：
  - 【普通工厂】：简单工厂和工厂方法都是面向一个接口来实现的，然而一个产品（e.g. Car）可能包含多个组成部分，那么有需要包含n多个不同的接口再进一步进行具体实现，此时普通方法模式就无法解决了。
  - 【抽象工厂】：围绕一个超级工厂创建其他工厂。该超级工厂又称为其他工厂的工厂。
  - 不可以增加产品，可以增加产品族。
  
**复杂度对比**：
1. **简单工厂 vs 工厂方法：**
- 结构负责度：simple factory
- 代码：simple
- 编程：simple
- 管理：simple
根据设计原则：工厂方法模式！
